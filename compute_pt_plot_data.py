...
def compute_group(name: str, scenarios, bin_type: str, output_dir: Path) -> None:
    ...
    np.savez(
        output_dir / f"{name}_fiducial.npz",
        ell_full=ell_full,
        cl_fid=cl_fid,
        ell=ell,
        bin_type=bin_type,
    ...
    print(f"Wrote {output_dir / f'{name}_fiducial.npz'}")
...
def main() -> None:
    ...
    if args.group in ("noisy", "both"):
        compute_group("noisy", NOISY, args.bin_type, output_dir)
